...
from ultralytics.nn.modules.lookup import LookupC2f, LookupC3k2, LookupConv, SmallObjectLookupConv, AdaptiveLookupConv
...
def parse_model(d, ch, verbose=True):  # model_dict, input_channels(3)
    ...
    repeat_modules = frozenset(  # modules with 'repeat' arguments
        {
            BottleneckCSP,YOLOv4_BottleneckCSP,
            C1,
            C2,
            C2f,
            C3k2,C3k2_DeepDBB,C3k2_DBB,C3k2_WDBB,C2f_DeepDBB,C2f_WDBB,C2f_DBB,C3k_RDBB,C2f_RDBB,C3k2_RDBB,A2C2f,DSC3k2,
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in base_modules :
            ...
            if m in (C3k2,):  # for M/L/X sizes
                ...
                if scale in "mlx":
                    args[3] = True
            if m in (A2C2f,) and scale in "lx": # for L/X sizes
                print('!!!')
        ...
        elif m in (LookupConv, SmallObjectLookupConv, AdaptiveLookupConv):  # for LookupNet Conv modules
            # 添加小目标检测模式参数和scale参数
            # args = [c1, c2, k, s, p, g, d, act, nf, nw, small_object_mode, scale]
            # 确保参数列表有足够的长度
            while len(args) < 12:
                if len(args) == 8:  # 只有基本参数，缺少nf, nw, small_object_mode, scale
                    args.extend([33, 33, False, scale])  # 添加nf, nw, small_object_mode, scale
                elif len(args) == 9:  # 缺少nw, small_object_mode, scale
                    args.extend([33, False, scale])  # 添加nw, small_object_mode, scale
                elif len(args) == 10:  # 缺少small_object_mode, scale
                    args.extend([False, scale])  # 添加small_object_mode, scale
                elif len(args) == 11:  # 缺少scale
                    args.append(scale)  # 添加scale
                else:
                    args.append(33)  # 添加默认值

            # 根据模型规模调整查找表大小
            if scale in "n":  # nano scale
                args[8] = 33  # nf
                args[9] = 33  # nw
            elif scale in "s":  # small scale
                args[8] = 49  # nf
                args[9] = 49  # nw
            elif scale in "mlx":  # medium/large/xlarge scale
                args[8] = 65  # nf
                args[9] = 65  # nw
                args[10] = True  # small_object_mode

        elif m in (LookupC2f, LookupC3k2):  # for LookupNet C2f/C3k2 modules
            # YAML参数格式: [c2, n, shortcut, g, e]
            # 构造函数期望: [c1, c2, n, shortcut, g, e, small_object_mode, scale]
            c1, c2 = ch[f], args[0]  # c1来自前一层，c2来自YAML第一个参数

            # 添加小目标检测模式参数和scale参数
            # args = [c1, c2, n, shortcut, g, e, small_object_mode, scale]
            args = [c1, c2, *args[1:]]  # 重新排列参数

            # 确保参数列表有足够的长度
            while len(args) < 8:
                if len(args) == 6:  # 缺少small_object_mode, scale
                    args.extend([True, scale])  # 添加small_object_mode, scale
                elif len(args) == 7:  # 缺少scale
                    args.append(scale)  # 添加scale
                else:
                    args.append(True)
